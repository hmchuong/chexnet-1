...
class LeNet():
  ...
  def conv_layers(self):
    ...
    with tf.name_scope('conv1') as scope:
      kernel = tf.Variable(tf.random_normal([3, 3, 1, 32], dtype=tf.float32, stddev=1e-1, # 3x3, # input channels, # output channels
        name='weights'))
    ...
    self.pool1 = tf.nn.max_pool(self.conv1,
      ksize=[1,2,2,1], # ..., 2 x 2 window, 1 channel
      strides=[1,2,2,1],
      padding='SAME',
      name='pool1')
    #self.dropout_cnn1 = tf.nn.dropout(self.pool1, keep_prob=self.keep_prob, name='dropout_cnn1')
    ...
    self.pool2 = tf.nn.max_pool(self.conv2,
      ...
      padding='SAME',
      name='pool2')
    #self.dropout_cnn2 = tf.nn.dropout(self.pool2, keep_prob=self.keep_prob, name='dropout_cnn2')
  ...
  def train(self, learning_rate, training_epochs, batch_size, keep_prob):
    ...
    save_path = saver.save(self.sess, "./model/worst_best_model.ckpt")
